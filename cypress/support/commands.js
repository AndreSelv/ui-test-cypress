...
Cypress.Commands.add("bootStrapOrg1", () => {
  ...
  cy.route({
    method: "GET",
    url: "/orgs",
    response: "fixture:orgs/orgs.json",
  });

  cy.route({
    method: "GET",
    url: "/orgs/org1",
    response: "fixture:orgs/org1/org1.json",
  });
  ...
  for (let i = 1; i <= 8; i++) {
    cy.fixture("orgs/org1/user").then((user) => {
      ...
      cy.route("GET", "/users/user" + i, user);
    });
  }
  ...
  cy.route({
    method: "GET",
    url:
      "/users?userIds=659a5e72-01c2-41dc-b962-e797b25d1636,user1,user2,user3,user4,user5,user6,user7,user8",
    response: "fixture:orgs/org1/users.json",
  });
});
